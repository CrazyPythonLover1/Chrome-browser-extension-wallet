  ...
  return (
    <>
      ...
      <Box sx={{ padding: "16px" }}>
        ...
        <Typography variant="body2" sx={{ lineHeight: "22px", marginTop: "4px", marginBottom: "14px" }}>
          ...
          {data.map((item) => (
            ...
            >
              <Box sx={{ display: "flex", alignItems: "center" }}>
                {item.icon}
                <Typography sx={{ marginLeft: "12px", fontWeight: "400" }}>{item.type}</Typography>
                {item.value && (
                  <Typography sx={{ marginLeft: "4px", fontSize: "12px", fontWeight: "400" }}>
                    ({item.value})
                  </Typography>
                )}
              </Box>
              <Box>
                <Button variant="grey" label="Add" onClick={() => navigate(item.path)} />
              </Box>
            </Box>
          ))}
        </Box>
